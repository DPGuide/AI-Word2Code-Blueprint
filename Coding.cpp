...
std::string ANSI_to_UTF8(const std::string& ansiStr) {
    ...
    // Null-Terminator sauber entfernen
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        case WM_CREATE: {
            ...
            hBar = CreateWindowExA(0, PROGRESS_CLASS, NULL, 
                        ...
                        10, 280, 300, 20, hWnd, NULL, NULL, NULL); // Leicht nach unten gerückt
        ...
        case WM_RBUTTONUP: {
            ...
            if (mx > 400) { 
                ...
                if (!clickedOnNode) {
                    ...
                    POINT pt;
                    GetCursorPos(&pt); // WICHTIG: Das Menü braucht Bildschirm-Koordinaten!
        ...
        case WM_PAINT: { // Wald, Raster & Kästchen zeichnen
            ...
            EndPaint(hWnd, &ps);
            return 0;
        } // <--- GENAU DIESE WICHTIGE KLAMMER HAT GEFEHLT!
            // -------------------------------------
        case WM_DRAWITEM: {
        ...
        case WM_COMMAND: {
			...
			if (LOWORD(wParam) == ID_BUTTON_COMPILE) {
                ...
                std::string userInstruction = ANSI_to_UTF8(buffer); // <--- HIER SCHÜTZEN WIR!
                ...
                ofn.Flags = OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT; // Fragt nach, wenn Datei schon existiert
                ...
                if (GetSaveFileNameA(&ofn) == TRUE) {
                    ...
                    if (outfile.is_open()) {
                        ...
                        if (!g_activeNodeID.empty() && g_allNodes.count(g_activeNodeID)) {
                             g_allNodes[g_activeNodeID].status = ColorMode::NEUTRAL; // Oder eine neue Farbe für "Gespeichert"
                             InvalidateRect(hWnd, NULL, FALSE);
                        }
                    } else {
            ...
            } // <-- BASTION endet hier (schließende Klammer für else if)
    ...
    } // <=== GANZ WICHTIG: Das hier schließt den "switch (message)" Block
    
    return DefWindowProcA(hWnd, message, wParam, lParam);
} // <=== GANZ WICHTIG: Das hier schließt die gesamte WndProc Funktion
